...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // === PENTING: Nama variabel harus 'unit_name' (sesuai nama kolom DB) ===
    ...
    // Query INSERT ke tabel 'units'
    // === PENTING: Nama kolom di query adalah 'unit_name' ===
    $sql = "INSERT INTO units (unit_name, description) VALUES ('$unit_name_safe', '$description_safe')";

    if ($conn->query($sql) === TRUE) {
        $_SESSION['message'] = "Satuan '<b>" . htmlspecialchars($unit_name) . "</b>' berhasil ditambahkan.";
        $_SESSION['message_type'] = "success";
        header("Location: units.php"); // Redirect ke halaman daftar satuan
        exit();
    } else {
        $_SESSION['message'] = "Error: " . $sql . "<br>" . $conn->error;
        $_SESSION['message_type'] = "danger";
        // Biasanya akan redirect juga, tapi bisa juga tetap di halaman ini dengan pesan error
...
<head>
    ...
    <title>Tambah Satuan Baru - Toko Sembako</title>
